...
- **P-6 PARTLY BANKED** `f26c77a` (2026-08-17): the ops directory caveats
  ...
  `20c0a22`. **REMAINING:** the shared `'Not authorized'` string (unsplit from
  the `/api/me` gate-state emission, per the sub-item below), the
  `CreateInviteModal` copy contradiction, the missing `/api/me` ops block, and
  the ops-minting guard.

...
| **Enterprise** | **Built and audited.** All 6 sections live (Overview, Roster, Program, Compliance, Reports (+ 5 sub-pages), Setup wizard). QA audit on branch `qa-audit-enterprise` (**157 findings** — 9 Critical + 33 High + 60 Medium + 55 Low — doc at `docs/qa-audit-enterprise-2026-05-30.md`; **branch unmerged** by design). **Count guard:** the audit's executive-summary "Medium: 76" is a typo (actual Medium = 60, verified by counting numbered lines); CLAUDE.md previously inherited the typo as "173 findings" — corrected here. The 26 Compliments listed in the audit are observations of working-well things, not findings, and are counted separately. **Enterprise does NOT use Advisor's numbered-bundle model.** Criticals + Highs shipped as direct per-finding-or-grouping commits; Mediums + Lows went through the cluster-based triage in `docs/qa-triage-medium-low-2026-05-30.md`. **9 Criticals shipped** (`33cb42f` covered all 9; `3bb9cb1` later completed Critical #9's full activity-log reconciliation as a side effect of fixing Medium #91). **26 of 33 Highs shipped — 7 still outstanding in code** (verified by grep against current main): #10 (HelpIcon hardcoded `#B8AE9E`), #11 (Tag.jsx accent-palette hardcoded `#F0EBDF/#5A554C/#D9C9B0`), #12 (Tag.jsx unused `warning` palette `#FCEAE0/#A03C18/#E8B6A1`), #13 (Button.jsx hardcoded `'#FFFFFF'`), #14 (SetupWizard stepper hardcoded `'#FFFFFF'`) — **5 brand-token Highs**; plus #15 (EnterpriseRoster `<tr onClick>` keyboard-inaccessible: no `tabIndex`/`role`/`onKeyDown` at line 74) and #16 (HelpIcon disclosure lacks `aria-expanded`/`aria-controls`) — **2 a11y Highs**. The 26 shipped Highs were addressed via `33cb42f` (Criticals bundle's secondary effects), `fd7b2b0` (Diane Okonkwo normalization, #17 + #29), `a525b29` (compliance officer + Andre/Ava + Mia/Jordan + CohortComparison yoy, #18/#21/#23/#24), `be6febc` (ISO dates + cohort framing, #19/#20), `aa56b64` (engagementTimeline derivation, #22), `d0e82fb` (React keys + responsive padding, #25-#28 + #32), `542e893` (ModalStack perf, #42), `bc0beb9` (persona canonical, #31 as side effect), and the `7998c7d` "narrative re-authoring" sweep that closed the voice/internal-leak cluster #30 + #33-#41 (10 items). **115 Mediums + Lows triaged** into 5 disposition buckets: **ALREADY-CLOSED (6)** — implemented incidentally before triage (includes #93 + #94 moved here from FIX-NOW after Cluster A shipped, per the triage doc's own Update note). **FIX-NOW (3 findings remaining post-Cluster-A-ship)** — all SHIPPED: Cluster B (#91 lesson-count reconciliation) via `3bb9cb1`; Cluster C (#76 Reports card "performance" + #82 W2 "stalled" voice) via `a1b505b`; Cluster A (#93 Chrome persona + #94 SetupWizard Diane) already shipped via `bc0beb9` and moved to ALREADY-CLOSED. **WON'T-FIX (41)** — audit-marked accept / by-design / verification-only observations, resolved by disposition (no code needed). **DEFER (5)** — revisit on specific later conditions (mobile QA pass, advisor Documentation rework, bundle-size threshold, etc.). **BATCH (60)** — dispositioned only, **0 shipped**: 9 clusters using Roman numerals — I (5: shared primitive extractions), II (7: dead code cleanup), III (2: perf tweaks), IV (11: token discipline), V (5: React keys / patterns), VI (18: voice / tone cleanup), VII (4: code quality minor), VIII (3: data consistency minor), IX (5: misc Low cleanup). **Naming-scheme note:** FIX-NOW uses LETTER clusters A/B/C (all shipped); BATCH uses ROMAN-NUMERAL clusters I–IX (open). CLAUDE.md previously said "Cluster A/B/C closed" which was true for FIX-NOW but obscured that the 60 BATCH items in I–IX remain unimplemented. **Audit posture: 90 of 157 dispositioned-or-shipped — 9 Critical + 26 High + 6 ALREADY-CLOSED + 3 FIX-NOW remaining + 41 WON'T-FIX + 5 DEFER. 67 outstanding implementation candidates — 7 unshipped Highs (5 brand-token + 2 a11y) + 60 BATCH Mediums/Lows.** The BATCH pile is the scope of any future Enterprise-depth arc; the brand-token Highs (#10-#14) pair naturally with BATCH Cluster IV (token discipline, 11 items) since the shape is identical. Fixture dates are uniformly shifted −192d for coherence with today (`Enterprise fixture date coherence` commit `7998c7d`). |

...
| **Individual** | **Paused at v0.6.1 — frozen, NOT audited as-is, full structural rework PENDING.** 15 surface files present (Discover, GPSReveal, GivingModeler, Plan, Positioning, Privacy, Team, History, Learn, CohortView, Letter, Questions, Feedback, GiveScreen, IndividualSurface). **Why no QA-arc audit:** Enterprise / Advisor / Operations got audit + fix-slice arcs because they were live-and-building (auditing compounded); Individual is **deliberately excluded** from that treatment because it is slated for a full structural rework, which would invalidate findings catalogued against the current code. **Disposition (founder-decided this session):** the 15 surfaces largely stay; **architecture / components / data-flow get restructured** — including adopting the shared primitives matured during the Enterprise arc (`DataTable`, `MessageHistoryCard`, the `Tag` `tracking='loose'` variant, the Enterprise-scoped `SegmentedControl`) and aligning to the unified data model at `src/data/unified/`. This is **REWORK (revise the existing surface), NOT REBUILD (discard to zero)**. **Escape hatch — explicit:** the rework-scoping pass may reveal the real question is scope/direction (what Individual should CONTAIN and DO in the three-sided model, reached through the advisor wedge) rather than structure. If so, the rework pivots from structural to scope/direction — that door is left open. **Structural is the primary frame; scope/direction is the live escalation path.** **Rework unblocks the 4 paused-surface rulings stacked during Enterprise** — all are Individual-gated and waiting on the thaw: **#12** (Tag `warning` palette consumed by paused `Team.jsx` — add `--sh-warning-bg/text/border` tokens when Individual resumes), **#63** (2 Individual `borderRadius` sites deferred from Sweep 1's Cluster IV token swap), **#116** (Card.jsx `interactive`/`onClick` infrastructure — 3 active consumers: `IndividualSurface.jsx:257`, `IndividualSurface.jsx:446`, `Learn.jsx:86`; InteractiveCard extraction or Card-keep ruling lands here). The rework should fold these in rather than ship them as separate slices. **Next step (NOT this session):** a read-only structural-rework SCOPING pass — map the 15 surfaces' current architecture, identify primitive-adoption opportunities and unified-data-model divergence, and flag any point where structural blurs into scope/direction (the escalation trigger). Founder-initiated; its own focused arc. |

...
- **Post-commit verification** (six checks as one pass/fail run; reads only,
  writes nothing, exits non-zero on any failure):
  ```sh
  node scripts/verify-commit-tail.mjs
  ```
  Trailers, the commit file list, the eight stated-vs-measured queue pairs
  against the COMMITTED blob, the derived identities, line endings for both
  working tree and blob, and the ignore rules. Every check prints its control
  beside its result.
...
**CORRECTED 2026-09-08: "nothing built" IS RETIRED. SLICE 1 IS BUILT.** The
sentence above is quoted rather than edited so the change is visible where the
stale claim sat. `scripts/bmf-fetch.mjs` acquires and freezes, `scripts/bmf-parse.mjs`
parses and emits, and `scripts/bmf-verify-slice1.mjs` loads and asserts. **Slices
2 and 3 are NOT started**, and A113 is not scoped beyond slice 1.
...
**CORRECTED 2026-09-10: the clause "and A113 is not scoped beyond slice 1" above
is SUPERSEDED**, quoted rather than edited so the change is visible where the
stale claim sat. **"Slices 2 and 3 are NOT started" STANDS** — neither has begun.
What no longer holds is the scoping half: **R32, ruled 2026-09-09, BOUNDS SLICE
2** as the whole loader, taking candidate 3 of the four boundaries published at
`docs/slice-2-scope-pass.md` §6 and naming thirteen elements A through M as
inside, with only R21c's three post-load operational items and the FT-run
...
A125's blocker-area RULED line, and A125's THIRD GROUND heading. All five are now
corrected. **Correcting four of five would have been worse than correcting
none**, and §7 above already names that shape: a surface that reads considered
while another copy still asserts the falsified claim. This one was found by
grepping for the clause rather than by working the list, which is the only method
that finds a fifth site nobody remembered writing.
**TWO FURTHER OCCURRENCES SURVIVE BY RULING AND ARE NOT A SIXTH SITE**, named
here so a later grep meets them as known rather than re-opening this: the prior
entry in `docs/session-log.md`, a historical record of what was true at that
session's close, and `docs/slice-2-scope-pass.md` §8, a dated publication whose
own snapshot note already yields to §15 and the queue where they disagree.
**Neither is a live claim**, and correcting a dated record would destroy the
evidence that the claim was once true.
...
- `.claude/agents/` — three Claude Code subagent definitions, tracked as of
  2026-09-08 by FT ruling (`.gitignore` carries `.claude/*` with a
  `!.claude/agents/` negation; the negation alone is insufficient because a
  trailing-slash exclusion stops git descending). **`builder`** scopes and builds
  on a working branch; **`adversary`** is read-only and looks for the claim that
  is wrong rather than reviewing; **`records`** maintains `docs/outstanding.md`
  and the queue arithmetic and files findings without ruling them. Each carries
  three absolutes in the file itself — never commit to `main`, never push, never
  run a remote D1 command — and each instructs the agent to treat every claim in
  a parent's delegation message as UNVERIFIED and re-prove it at HEAD.

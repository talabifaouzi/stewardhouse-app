...
- **P-6 PARTLY BANKED** `f26c77a` (2026-08-17): the ops directory caveats
  ...
  and the ops-minting guard at `537cc08`. **REMAINING, and they are ONE
  slice:** the missing `/api/me` ops block, now unblocked by the boolean
  ruling two sub-items below, and the shared `'Not authorized'` string ruled
  UNSPLIT from that emission.
- **P-6 sub-item, named so it is not rediscovered: the shared `'Not authorized'`
  ...
  gate, `:107` non-staff, `:119` enterprise gate, `:156` / `:177` non-ops, `:189`
  ops gate), so it collapses two orthogonal failures: *wrong account type* and
  *account not designated for writes*. `jsonError` (`:194-198`) passes it to the

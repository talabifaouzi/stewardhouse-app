...
- **Tier 3 remaining**: **#116 InteractiveCard design call** — the LAST paused-surface fold (3 Card `interactive`/`onClick` consumers — IndividualSurface ×2 + Learn; extract vs Card-keep dual-mode); **further unified-data-model adoption** if/when prioritized (Section 3 divergences #2-5: individualProfile-raw-vs-unified, intake-gifts-join-unified, ID-namespacing in IntakeContext writes, `CohortMemberContext`-in-unified).

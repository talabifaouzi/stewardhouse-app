...
- ~~Primitive-adoption opener: #12 warning-palette fold (4 sites)~~ → **DONE** (`cd1a518`) — landed at **10 sites, not 4** (6 past-due literals surfaced during the slice via grep; see §4 #12 entry). Real DRY win banked. **Next Tier 3 item is unified-data-model adoption (CohortView first — Section 3 divergence #1).**
...
**Scoping-count discipline (added after the #12 fold landed)**: investigation has expanded Tier 3 scope beyond initial grep **twice** — the **12-pill correction** (`b0f0e0c`: 12 anticipated → 0 valid sites; the original grep matched pill-shaped INTERACTIVE controls rather than static informational pills) and the **#12 fold** (`cd1a518`: 4 anticipated → 10 actual; the broader past-due hex literals surfaced only during the slice's own grep sweep). **Tier 3 scoping counts are FLOORS, not ceilings.** Expect scope-discovery during each remaining slice's investigation pass — plan investigation time for it; don't promise the scoping count as the final count until the slice's own grep sweep is done.

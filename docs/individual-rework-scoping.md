...
| # | Sites | Fold-in plan |
|---|---|---|
| **#12** Tag `warning` palette | **4 sites (updated this session)** — 3 existing `<Tag tone="warning">` consumers: Team.jsx:338 + :380 `Past due`, Team.jsx:414 `Missing`; **+1 newly-discovered hand-rolled sibling**: Team.jsx:127 (tab-count badge) hand-rolls the same `#F8D7CC` / `#A03C18` hex pair already in Tag.jsx's `warning` scheme — a hidden #12 site found during the 12-pill investigation | Add 3 tokens to `tokens.css`: `--sh-warning-bg`, `--sh-warning-text`, `--sh-warning-border` (currently hex `#FCEAE0` / `#A03C18` / `#E8B6A1` in Tag.jsx). Tag.jsx swap is 3 lines; the 3 existing `<Tag tone="warning">` consumers stay byte-unchanged. Migrate Team.jsx:127 from its hand-rolled hex literals to `<Tag tone="warning">` after the token swap, so all 4 sites converge on the token-backed scheme. Pair with Team.jsx rework to avoid two-pass churn. |
...
- **Primitive-adoption opener (replaces the skipped 12-pill migration): #12 warning-palette fold (4 sites)** — 3 tokens (`--sh-warning-bg/text/border`) + Tag.jsx scheme swap (byte-unchanged for the 3 existing `<Tag tone="warning">` consumers) + migrate Team.jsx:127 hand-rolled hex literals to `<Tag tone="warning">`. Real DRY win.
- **Unified-data-model adoption** *(unchanged — the structural bulk)* — sequence: CohortView first (5.7 / Section 3 divergence #1), then the gifts / ID-namespacing / context decisions (Section 3 divergences #2-5).
- **3 paused-surface folds** — **#12 now sized at 4 sites** (3 tokens + Tag swap + Team:127 migration); #63 (radius var); #116 (InteractiveCard design call).
